...
const executable_name =
	machine_os === "win" ? "N_m3u8DL-RE.exe" : "N_m3u8DL-RE";
if (fs.existsSync(executable_name)) {
	scrapeSite();
} else {
	throw Error(
		"N_m3u8DL-RE binary not found! Download the Binary here: https://github.com/nilaoda/N_m3u8DL-RE/releases"
	);
}
...
async function downloadVideo(vData, title, unitTitle, index) {
	if (
		!fs.existsSync(`domestika_courses/${title}/${vData.section}/${unitTitle}/`)
	) {
	...
	try {
		if (machine_os === "win") {
			let log = await exec(
				`N_m3u8DL-RE -sv res="1080*":codec=hvc1:for=best "${
					vData.playbackURL
				}" --save-dir "domestika_courses/${title}/${
					vData.section
				}/${unitTitle}" --save-name "${index}_${vData.title.trimEnd()}"`,
				options
			);
			let log2 = await exec(
				`N_m3u8DL-RE --auto-subtitle-fix --sub-format SRT --select-subtitle lang="${subtitle_lang}":for=all "${
					vData.playbackURL
				}" --save-dir "domestika_courses/${title}/${
					vData.section
				}/${unitTitle}" --save-name "${index}_${vData.title.trimEnd()}"`,
